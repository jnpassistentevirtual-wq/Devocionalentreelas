...
async function loadMural() {
  ...
  const [annRes, commRes, prayRes, testRes, worshipRes, topicsRes] = await Promise.all([
    sb.from("mural_announcement").select("text").eq("id", 1).maybeSingle(),
    sb.from("mural_comments").select("*").order("created_at", { ascending: true }),
    sb.from("mural_prayers").select("*").order("created_at", { ascending: true }),
    sb.from("mural_testimonies").select("*").order("created_at", { ascending: true }),
    sb.from("mural_worship").select("*").order("created_at", { ascending: true }),
    sb.from("mural_prayer_topics").select("*").order("id", { ascending: true }),
  ]);
...
async function saveAnnouncement() {
  await sb.from("mural_announcement").update({ text: state.mural.announcementDraft, updated_at: new Date().toISOString() }).eq("id", 1);
...
async function postComment() {
  ...
  if (!name || !text) return;
  await sb.from("mural_comments").insert({ name, text });
...
async function postPrayer() {
  ...
  if (!name || !text) return;
  await sb.from("mural_prayers").insert({ name, text, pray_count: 0 });
...
async function incrementPray(id) {
  ...
  if (!prayer) return;
  await sb.from("mural_prayers").update({ pray_count: (prayer.pray_count || 0) + 1 }).eq("id", id);
  await loadMural();
}
...
async function postTestimony() {
  ...
  if (!name || !text) return;
  await sb.from("mural_testimonies").insert({ name, text });
...
async function postWorship() {
  ...
  if (!name || !text) return;
  await sb.from("mural_worship").insert({ name, text });
  state.mural.worshipText = "";
  await loadMural();
}

async function saveTopicEdit(id) {
  await sb.from("mural_prayer_topics").update({ prayer: state.mural.topicDraft, updated_at: new Date().toISOString() }).eq("id", id);
